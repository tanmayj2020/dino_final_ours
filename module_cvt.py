...
class PreNormIm(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.fn = fn
    def forward(self, x,img_size ,**kwargs):
        return self.fn(self.norm(x),img_size, **kwargs)
...
class ConvAttention(nn.Module):
    ...
    def forward(self, x , img_size):
        ...
        x = rearrange(x, 'b (l w) n -> b n l w', l=img_size, w=img_size)
        q = self.to_q(x)
